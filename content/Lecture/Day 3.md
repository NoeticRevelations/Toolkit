...
graph LR
    ...
    %% Meaning Details
    M --> M1["'My life is over now I am retired'"]
    M --> M2["'When I quit my job I lost part of myself'"]

    %% Expectation Details
    E --> E1["'All I can see is my life is black, black, black'"]
    E --> E2["'I've just got to re-adjust my life and do what I can'"]
    E --> E3["Perspective in acceptance of life with diabetes <br/>(Rasmussen et al., 2007)"]

    %% Knowledge Details
    K --> K1["Knowledge about transition & personal resources to cope <br/>(Halding & Heggdal, 2012)"]

    %% Resources Details
    R --> R1["Social support, community orgs, groups, clubs <br/>(Furnes et al., 2014)"]
    R --> R2["Meaningful relationships <br/>(Rasmussen et al., 2007; Schumacher & Meleis, 1994)"]

    %% Capacity Details
    C --> C1["Ability to prepare (Expected vs Unexpected)"]
    C --> C2["Unexpected health events <br/>e.g., stroke or heart attack"]
...
```mermaid
graph TD
    A["Initially focus upon short-term goals"] --> B["Set no more than 3 goals to begin with"]
    B --> C["Ensure goals are positively set"]
...
| **O** | Training regularly<br>Improving technique | Repetition, writing something down to process more deeply | Learning new strategies and practicing these | Investing time and effort into meaningful relationships |
| **C** | Using supports e.g. muscle-support tape<br>Use of lighter weights | Using phone reminders or calendars | Asking for help if overwhelmed | Using texting/ social media for frequent contact if unable to regularly meet in person |
...
```mermaid
graph TD
    %% Class definitions for specific colors and rounded corners based on the image
    %% Yellow-Orange (Feeling Better): #FFBF00
    %% Green (Boom): #4EE34E
    %% Teal (Increased Symptoms): #30CC8D
    %% Blue (Bust): #6699CC

    classDef feeling-better-day fill:#FFBF00,stroke:#333,stroke-width:1px,rx:10,ry:10,color:#fff;
    classDef boom-overactivity fill:#4EE34E,stroke:#333,stroke-width:1px,rx:10,ry:10,color:#fff;
    classDef increased-symptoms fill:#30CC8D,stroke:#333,stroke-width:1px,rx:10,ry:10,color:#fff;
    classDef bust-prolonged-rest fill:#6699CC,stroke:#333,stroke-width:1px,rx:10,ry:10,color:#fff;

    %% Define nodes with text content and correct formatting with line breaks
    %% Order is set to allow a clean top-down flow while maintaining the logical cycle
    %% B-Left (Blue), T-Top (Yellow), T-Right (Green), B-Bottom (Teal)
    
    A[A 'feeling better' day]:::feeling-better-day
    B("'Boom'<br>Over-activity"):::boom-overactivity
    C(Increased<br>symptoms):::increased-symptoms
    D("'Bust'<br>Prolonged<br>rest"):::bust-prolonged-rest

    %% Define circular process connections
    A --> B
    B --> C
    C --> D
    D --> A

    %% Applying custom stroke styles to edges for a thinner, lighter look (similar to original image)
    linkStyle 0,1,2,3 stroke:#AAAAAA,stroke-width:1px;
```
